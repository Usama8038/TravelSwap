...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden" style={{ background: '#0a0a0a' }}>
      ...
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full opacity-20 blur-[120px]" style={{ background: 'var(--color-primary)' }}></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] rounded-full opacity-10 blur-[120px]" style={{ background: 'var(--color-secondary)' }}></div>
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-extrabold mb-2 tracking-tight text-white">Access the Exchange</h1>
          <p className="text-sm text-gray-400">Enter your credentials to manage your mobility assets</p>
        </div>
        ...
        <div className="p-10 rounded-[2.5rem] backdrop-blur-xl border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]" 
          ...
          <button className="w-full py-4 rounded-2xl text-sm font-semibold flex items-center justify-center gap-3 mb-8 transition-all duration-300 hover:bg-white/5 border border-white/5 hover:border-white/20 text-white"
            onClick={() => toast.success('Google OAuth is pending environment configuration.')}>
          ...
          <div className="relative mb-8">
            ...
            <div className="relative flex justify-center"><span className="px-4 text-[10px] uppercase tracking-widest bg-[#141414] text-gray-500 font-bold">Secure Email Login</span></div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end">
              <Link href="/forgot-password" title="Recover Access" className="text-xs font-semibold text-[var(--color-primary)] hover:brightness-125 transition-all no-underline">Forgot password?</Link>
            </div>
            <button type="submit" disabled={loading}
              ...
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> : <><span>Enter Dashboard</span><ArrowRight className="w-5 h-5" /></>}
            </button>
          </form>
        </div>

        <p className="text-center text-sm mt-8 text-gray-500">
          New to the exchange? <Link href="/signup" className="text-white font-bold no-underline hover:text-[var(--color-primary)] transition-colors">Create account</Link>
        </p>
      </motion.div>
    </div>
  );
